...
/**
 * Remove duplicate copies of any track from a playlist, keeping the FIRST
 * occurrence's position. A playlist must never hold the same song twice
 * (WHI-1355: Rise testing stacked one song 6x at the top of three playlists).
 * Returns { removed, tracks: [{id, name, copies}] }.
 */
export async function removeDuplicateCopies(playlistId) {
  const tracks = await getPlaylistTracks(playlistId);
  const seen = new Map(); // id -> { uri, name, extra: count of extra copies }
  for (const t of tracks) {
    if (!seen.has(t.id)) seen.set(t.id, { uri: t.uri, name: t.name || '', extra: 0 });
    else seen.get(t.id).extra += 1;
  }
  const dupes = [...seen.entries()].filter(([, v]) => v.extra > 0);
  if (!dupes.length) return { removed: 0, tracks: [] };
  // Spotify's positional DELETE ({uri, positions, snapshot_id}) returns 200 but
  // no longer removes anything (verified live 2026-09-07), so: remove EVERY copy
  // by uri, re-add one, and move it back to where the first copy sat. The
  // Spotify "added" date of that one track resets; the curator's own date store
  // (authoritative for Refresh tiers) is untouched.
  // Move offsets must be PHYSICAL item positions (local files / episodes count),
  // so positions come from the raw item list, not the filtered track list.
  const rawUris = async () => {
    const out = [];
    let url = `/playlists/${playlistId}/tracks?limit=100&fields=items(track(uri)),next`;
    while (url) {
      const res = await api(url);
      if (!res.ok) throw new Error(`removeDuplicateCopies: raw fetch failed ${res.status}`);
      const data = await res.json();
      for (const it of data.items || []) out.push((it.track && it.track.uri) || null);
      url = data.next ? data.next.replace(API, '') : null;
    }
    return out;
  };
  ...
  for (const [, v] of dupes) {
    const now = await rawUris();
    const firstPos = now.indexOf(v.uri);
    const copies = now.filter((u) => u === v.uri).length;
    if (copies < 2) continue;
    await removeTracks(playlistId, [v.uri]);
    await appendTracks(playlistId, [v.uri]);
    const after = await rawUris();
    const lastPos = after.length - 1;
    // Concurrency guard: only move if the re-added copy is exactly where we
    // expect (last), is now the sole copy, AND everything above the original
    // first position is unchanged (so firstPos is still valid); else leave it
    // at the bottom and report.
    const prefixSame = firstPos >= 0 && firstPos <= after.length &&
      now.slice(0, firstPos).every((u, i) => after[i] === u);
    if (after[lastPos] !== v.uri || after.indexOf(v.uri) !== lastPos || !prefixSame) {
      report.push({ id: v.uri.split(':').pop(), name: v.name, copies, warning: 'playlist changed mid-dedupe; copy left at bottom' });
      removed += copies - 1;
      continue;
    }
    if (firstPos >= 0 && lastPos > firstPos) {
      const res = await api(`/playlists/${playlistId}/tracks`, {
        method: 'PUT', body: { range_start: lastPos, insert_before: firstPos, range_length: 1 },
      });
      if (!res.ok) { const t = await res.text(); throw new Error(`removeDuplicateCopies move failed: ${res.status} — ${t}`); }
    }
    removed += copies - 1;
    report.push({ id: v.uri.split(':').pop(), name: v.name, copies });
  }
  return { removed, tracks: report };
}
...
async function applyCuration(job, mode) {
  ...
  // No duplicates, ever (WHI-1355): drop extra copies FIRST so the protected
  // top N is N distinct songs, not one song repeated.
  const dedup = await removeDuplicateCopies(job.targetPlaylistId);
  ...
  // Pin by SLOT COUNT, not by id (WHI-1338): a playlist can hold the same track
  // several times (e.g. a client song repeated 6x at the top). Filtering the pool
  // by id dropped the extra copies, so desired.length !== live.length and
  // reorderToMatch silently skipped ("membership mismatch") -> refresh never ran.
  const pinSlots = new Map();
  for (const p of pinned) pinSlots.set(p.id, (pinSlots.get(p.id) || 0) + 1);
  const livePinned = [];
  const pool = [];
  for (const t of live) {
    const n = pinSlots.get(t.id) || 0;
    if (n > 0) { pinSlots.set(t.id, n - 1); livePinned.push(t); } else pool.push(t);
  }
  ...
  const out = {
    ...
    added: brandNew.length, reAdded: reAdd.length, removedFromSource: removed, moves: reorder.moves,
    dedupedCopies: dedup.removed, ...info,
  };
  out.message =
    mode === 'refresh'
  ...
  if (dedup.removed) out.message += ` Removed ${dedup.removed} duplicate cop${dedup.removed === 1 ? 'y' : 'ies'} (${dedup.tracks.map((t) => `${t.name} x${t.copies}`).join(', ')}).`;
  if (reorder.skipped) { out.reorderSkipped = reorder.skipped; out.message += ` WARNING: reorder skipped (${reorder.skipped}).`; }
  return out;
}
