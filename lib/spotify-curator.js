...
export async function curateOnce(job) {
  return job.mode === 'resort' ? runResort(job) : runInsert(job);
}
...
export async function runInsert(job) {
  ...
  const have = new Set(targetTracks.map((t) => t.id));
  const seen = new Set();
  const newTracks = [];
  for (const t of sourceTracks) {
    if (!have.has(t.id) && !seen.has(t.id)) { seen.add(t.id); newTracks.push(t); }
  }

  if (newTracks.length === 0) {
    return { ok: true, mode: 'insert', added: 0, total: targetTracks.length, message: 'No new submissions to add.' };
  }

  const energy = await getEnergyMap([...targetTracks, ...newTracks].map((t) => t.id));
  const working = withEnergy(targetTracks, energy);
  // Insert higher-ranked new tracks first so multiple adds nest correctly.
  const incoming = withEnergy(newTracks, energy)
    .sort((a, b) => energyRank(b.e, direction) - energyRank(a.e, direction));

  const placements = [];
  for (const nt of incoming) {
    const rank = energyRank(nt.e, direction);
    const floor = Math.min(excludeTopN, working.length); // never above protected top
  ...
  return {
    ...
    message: `Inserted ${incoming.length} new track(s) by energy (add-only; top ${excludeTopN} protected; ${placement === 'window' ? `within first ${windowSize}` : 'throughout'}). Target now ${working.length} tracks.`,
  };
}
