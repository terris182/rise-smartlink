...
export async function buildSuggestions(key) {
  ...
  for (const t of viralRows) {
    ...
    // Match on the PRIMARY (first) artist's genres only. A collaboration's
    // FEATURED artist must never pull an off-genre song onto a page
    // (WHI-1335, 2026-09-21: "House featuring John Cale" by Charli xcx, a
    // hyperpop song, was landing on the country/folk page because the
    // featured John Cale reads as "singer-songwriter"). The primary artist
    // is who the song belongs to, so the page fit is decided by them, and
    // the displayed genres come from them too rather than the union.
    const primaryId = t.artistIds[0];
    const primaryGenres = genreMap.get(primaryId) || [];
    if (!matchesGenre(primaryGenres, cfg.genres)) continue;
    seen.add(t.id);
    viral.push({ ...t, genres: Array.from(new Set(primaryGenres)).slice(0, 4) });
