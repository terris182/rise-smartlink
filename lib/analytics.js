...
/**
 * Get today's date string in PST/PDT (America/Los_Angeles).
 * This ensures daily buckets align with Terris's local timezone.
 */
function getToday() {
...
export async function getAnalytics(slug) {
  ...
  try {
    ...
    // Use PST dates to match how data is stored
    const daily = {};
    const now = new Date();
    const dailyPromises = [];
